...
class PathwayView(QWidget):
    ...
    def setup_header(self):
        ...
        v = QVBoxLayout(); t1 = QLabel("Metabolic Pathway Reconstruction"); t1.setStyleSheet("color: #2B3674; font-size: 22px; font-weight: 900;")
        t2 = QLabel("KEGG Mapper • Enzyme Annotation • Functional Module Analysis"); t2.setStyleSheet("color: #A3AED0; font-weight: 600; font-size: 12px;")
    ...
    def setup_pipeline_tracker(self):
        ...
        steps = ["GENE SCAN", "EC CODES", "KEGG MAP", "MODULES", "REPORT"]
    ...
    def setup_controls(self):
        ...
        btn_browse = QPushButton("📂 Load Genome"); btn_browse.setObjectName("btn_browse"); btn_browse.setFixedSize(140, 50)
        ...
        self.btn_run = QPushButton("RECONSTRUCT PATHWAYS"); self.btn_run.setObjectName("btn_run"); self.btn_run.setFixedHeight(50)
    ...
    def setup_visuals(self):
        ...
        l1 = QLabel("FUNCTIONAL ANNOTATION RESULTS"); l1.setObjectName("section_header"); vl.addWidget(l1)
        r1 = QHBoxLayout(); r1.setSpacing(15)
        # KEEP DATA CARDS COLORFUL
        self.map_card = self.create_stat_card("Enzymes Mapped", "🧩", "#7B61FF")
        self.comp_card = self.create_stat_card("Completeness", "📈", "#05CD99")
        self.top_card = self.create_stat_card("Dominant Pathway", "🔥", "#FFAB00")
        r1.addWidget(self.map_card[0]); r1.addWidget(self.comp_card[0]); r1.addWidget(self.top_card[0]); vl.addLayout(r1)
        self.card_layout.addWidget(self.vis_container)

    def setup_terminal(self):
        l = QLabel("RECONSTRUCTION LOGS"); l.setObjectName("section_header"); self.card_layout.addWidget(l)
        self.terminal = QTextEdit(); self.terminal.setObjectName("terminal"); self.terminal.setReadOnly(True)
        self.terminal.setPlaceholderText("Ready. Load a genome to map metabolic networks..."); self.terminal.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
    ...
    def update_results(self, data):
        self.map_card[1].setText(f"{data['mapped']:,}"); self.comp_card[1].setText(data['completeness']); self.top_card[1].setText(data['top_pathway']); self.top_card[1].setStyleSheet("font-size: 16px; font-weight: 700; color: #FFAB00;")
